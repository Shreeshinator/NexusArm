# Recording a LeRobot v3 Dataset from ROS 2 Topics (Simulated Arm)

This document explains how to record a [LeRobotDataset v3](https://huggingface.co/docs/lerobot/main/en/lerobot-dataset-v3)
directly from ROS 2 topics — two camera streams (gripper + third-person) and joint
angles — without a real robot. It contains a complete, ready-to-run recorder node.
Every place where you will likely need to adapt something to your setup is marked
with a `>>> CUSTOMIZE` comment in the code.

---

## 1. How it works

LeRobot's recording pipeline is just a Python API — `lerobot-record` and real
hardware are optional. The recorder node below:

1. Subscribes to your camera topics (`sensor_msgs/Image` or `CompressedImage`),
   `/joint_states`, and optionally a joint **command** topic (used as the `action`).
2. Caches the *latest* message from each topic.
3. A fixed-rate timer (`fps`) snapshots the caches into `dataset.add_frame(...)`.
   This is important: LeRobot assumes a **constant frame rate** (frame timestamps
   are `index / fps`), so we sample on a clock instead of recording every message.
4. Episode boundaries are controlled interactively from the keyboard **and** via a
   `std_msgs/String` command topic, so a sim orchestration script can drive
   recording automatically (start → rollout → save → reset sim → repeat).
5. On exit it calls `dataset.finalize()` — **mandatory in v3**, otherwise the
   parquet files are corrupt — and optionally `push_to_hub()`.

```
 /gripper_cam/image_raw ──┐
 /front_cam/image_raw ────┤   latest-message      30 Hz timer          per episode
 /joint_states ───────────┼──►   caches      ──►  add_frame()  ──►  save_episode()
 /joint_commands ─────────┘                                              │
                                                                    finalize()  →  push_to_hub()
```

## 2. Prerequisites

```bash
# ROS 2 (any recent distro; tested API is Humble/Jazzy-era rclpy)
sudo apt install ros-$ROS_DISTRO-cv-bridge   # optional; the node has a manual fallback

# LeRobot v3 needs lerobot >= 0.4.0 (or main branch)
pip install "lerobot>=0.4.0"
# or: pip install git+https://github.com/huggingface/lerobot.git

pip install opencv-python numpy

# Only if you want to push to the Hub:
huggingface-cli login
```

> **Note:** run the node from a shell where both your ROS 2 environment
> (`source /opt/ros/<distro>/setup.bash`) and the Python env containing
> `lerobot` are active. If your ROS Python and lerobot venv conflict, the
> cleanest fix is a venv created with `--system-site-packages`.

## 3. The dataset schema

The node records these features (keys follow LeRobot conventions so training
configs work out of the box):

| Key | dtype | shape | Source |
|---|---|---|---|
| `observation.state` | float32 | `(N_joints,)` | `/joint_states` positions |
| `action` | float32 | `(N_joints,)` | command topic (or state fallback) |
| `observation.images.gripper` | video | `(H, W, 3)` | gripper camera |
| `observation.images.front` | video | `(H, W, 3)` | third-person camera |

Image shapes are auto-detected from the first received message, so you don't
have to hardcode resolutions — but each camera's resolution must stay constant
for the whole dataset.

## 4. The recorder node

Save as `lerobot_ros2_recorder.py`.
...
```bash
# Terminal 1: your simulation (Gazebo / Isaac / MuJoCo bridge / ...)

# Terminal 2: the recorder
python3 lerobot_ros2_recorder.py \
  --repo-id your-hf-username/sim-arm-pick-cube \
  --task "pick up the red cube" \
  --fps 30 \
  --gripper-cam-topic /gripper_cam/image_raw \
  --front-cam-topic /front_cam/image_raw \
  --joint-states-topic /joint_states \
  --joint-commands-topic /joint_commands \
  --joint-names shoulder_pan shoulder_lift elbow wrist_1 wrist_2 wrist_3 \
  --push
```

Then per episode: press **ENTER** to start, run your rollout in the sim, press
**ENTER** again to save (or `d` to discard a failed rollout), reset the sim, and
repeat. `q` finishes, finalizes, and pushes.

### Driving it from a sim script (no keyboard)

```bash
ros2 topic pub --once /lerobot_recorder/command std_msgs/String "{data: start}"
# ... run rollout ...
ros2 topic pub --once /lerobot_recorder/command std_msgs/String "{data: save}"
# ... reset sim, loop ...
ros2 topic pub --once /lerobot_recorder/command std_msgs/String "{data: finish}"
```

Or from Python inside your rollout script with a `rclpy` publisher — start the
episode after the sim reset settles, save after the success condition triggers.
...
ds = LeRobotDataset("your-hf-username/sim-arm-pick-cube")
print(ds.meta.info)          # schema, fps, episode count
print(ds.num_episodes, ds.num_frames)
sample = ds[0]
print(sample["observation.state"], sample["action"])
print(sample["observation.images.gripper"].shape)   # [3, H, W] float tensor
```

You can also eyeball episodes with the online visualizer once pushed:
`https://huggingface.co/spaces/lerobot/visualize_dataset` (enter your repo id).

## 7. Gotchas & tips

- **`finalize()` is not optional.** v3 writes parquet incrementally; without it
  the files lack footers and the dataset won't load. The node calls it in the
  `finally` block, so even Ctrl-C keeps already-saved episodes usable.
- **fps vs. publish rates.** Sample at or below your slowest camera's rate. If a
  camera publishes at 15 Hz but you record at 30 fps, every image appears twice
  and the video stream lies about dynamics.
- **Sim time.** If your sim runs slower/faster than real time, the wall-clock
  timer drifts relative to sim dynamics. For non-realtime sims, prefer driving
  frame capture from the sim loop itself (publish a "tick" and record on it), or
  run the node with `use_sim_time` and a `/clock`-driven timer.
- **Action semantics.** For imitation learning you almost always want the
  *commanded* target, not the measured state — measured state lags the command
  and a policy trained on it will act "one step behind". If you truly only have
  states, consider post-shifting: `action[t] = state[t+1]`.
- **Latency alignment.** All sources are sampled "latest wins". At 30 fps that's
  usually fine in sim. If you need tighter sync, compare `header.stamp` values
  across cached messages and log the spread.
- **Storage.** Videos encode in the background; a few hundred 640×480 episodes
  are typically only a few GB. Default local root is
  `~/.cache/huggingface/lerobot/<repo-id>` (override with `--root`).
