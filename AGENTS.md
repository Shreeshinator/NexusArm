...
## Commit messages
- Write them like a teammate would: one short, plain, human sentence in the present tense saying what changed. No `fix:`/`docs:`/`feat:` prefixes, and **no description/body** — just the single line.
- Keep it under ~70 chars. The diff explains the rest; don't pad it.
- Examples:
  - `fixed the Hugging Face badge so the logo is readable`
  - `switched the Python packages to exec_depend`
  - `stopped tracking AGENTS.md`
  - `added a docs roadmap so readers know what to read next`
...
- Recorder script: `lerobot-ros2-recorder.py` at repo root — see `docs/05_DATA_COLLECTION.md` for usage.
